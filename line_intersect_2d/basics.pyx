...
cdef class Segment:
    ...
    cpdef Point intersection_point(self, Segment s):
        """
        .. versionadded:: 0.2
        ...
        cdef:
            ...
            double s2_x = s.stop.x - s.start.x
            double s2_y = s.stop.y - s.start.y
            double s = (-s1_y * (self.start.x - s.start.x) + s1_x * (self.start.y - s.start.y)) / \
                    (-s2_x * s1_y + s1_x * s2_y)
            double t = (s2_x * (self.start.y - s.start.y) - s2_y * (self.start.x - s.start.x)) / \
                       (-s2_x * s1_y + s1_x * s2_y)
